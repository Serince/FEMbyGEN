    ...
    def main(self):
        ...
        for ft in self.filter_list:  # find if automatic filter range is used
            if ft[0] and (ft[1] == "auto") and not self.filter_auto:
                size_elm = self.beso_filters.BesoFilters(Elements,nodes).find_size_elm(elm_category)
            ...
            if ft[0] and ft[1]:
                ...
                if ft[0] == "casting":
                    if len(ft) == 3:
                        ...
                        self.beso_filters.BesoFilters.check_same_state(
                        ...
                        self.beso_filters.BesoFilters.check_same_state(self.domain_same_state, filtered_dn, self.file_name)
                    casting_vector = ft[2]
                    ...
                    if f_range == "auto":
                        size_avg = self.beso_filters.BesoFilters.get_filter_range(size_elm, domains, filtered_dn)
                        f_range = size_avg * 2
                        msg = "Filtered average element size is {}, filter range set automatically to {}".format(size_avg,
                                                                                                                 f_range)
                        print(msg)
                        self.beso_lib.write_to_log(self.file_name, msg)
                    [above_elm, below_elm] = self.beso_filters.BesoFilters.prepare2s_casting(cg, f_range, domains_to_filter,
                ...
                if f_range == "auto":
                    ...
                    self.beso_lib.write_to_log(self.file_name, msg)
                if ft[0] == "over points":
                    self.beso_filters.BesoFilters.check_same_state(self.domain_same_state, self.domains_from_config, self.file_name)
                    [w_f3, n_e3, n_p] = self.beso_filters.BesoFilters.prepare3_tetra_grid(
                    ...
                    self.beso_filters.BesoFilters.check_same_state(self.domain_same_state, self.domains_from_config, self.file_name)
                    [w_f_n, M_, w_f_d, n_n] = self.beso_filters.BesoFilters.prepare1s(
                    ...
                    [weight_factor2, near_elm] = self.beso_filters.BesoFilters.prepare2s(cg, cg_min, cg_max, f_range, domains_to_filter,
                                                                             self.weight_factor2, self.near_elm)
                elif ft[0].split()[0] in ["erode", "dilate", "open", "close", "open-close", "close-open", "combine"]:
                    near_elm = self.beso_filters.BesoFilters.prepare_morphology(
                ...
                if ft[0] and ft[1]:
                    if ft[0] == "casting":
                        if len(ft) == 3:
                            ...
                            for dn in ft[3:]:
                                domains_to_filter += domains[dn]
                        sensitivity_number = self.beso_filters.BesoFilters.run2_casting(sensitivity_number, above_elm, below_elm,
                                                                            domains_to_filter)
                        continue  # to evaluate other filters
                    if len(ft) == 2:
                        domains_to_filter = list(opt_domains)
                        ...
                        for dn in ft[2:]:
                            domains_to_filter += domains[dn]
                    if ft[0] == "over points":
                        sensitivity_number = self.beso_filters.BesoFilters.run3(sensitivity_number, self.weight_factor3[kp], self.near_elm3[kp],
                                                                    self.near_points[kp])
                        kp += 1
                    elif ft[0] == "over nodes":
                        sensitivity_number = self.beso_filters.BesoFilters.run1(self.file_name, sensitivity_number, self.weight_factor_node[kn], self.M[kn],
                        ...
                        if ft[0].split()[1] == "sensitivity":
                            sensitivity_number = self.beso_filters.BesoFilters.run_morphology(sensitivity_number, near_elm, domains_to_filter,
                                                                                  ft[0].split()[0])
                ...
                if ft[0] and ft[1]:
                    ...
                    if ft[0].split()[0] in ["erode", "dilate", "open", "close", "open-close", "close-open", "combine"]:
                        if ft[0].split()[1] == "state":
                            ...
                            elm_states_filtered = self.beso_filters.BesoFilters.run_morphology(elm_states, near_elm, domains_to_filter,
