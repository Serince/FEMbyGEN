...
# function to check if filtering is to be used on domains with prescribed same state
    def check_same_state(self, domain_same_state, filtered_dn, file_name):
        wrong_domains = False
...
# function preparing values for filtering element sensitivity numbers to suppress checkerboard
    def prepare1(self, cg, r_min, opt_domains):
        ...
        for en in self.Elements.tria3:
            for nn in self.Elements.tria3[en]:
                fce()
        for en in self.Elements.tria6:
            for nn in self.Elements.tria6[en]:
                fce()
        for en in self.Elements.quad4:
            for nn in self.Elements.quad4[en]:
                fce()
        for en in self.Elements.quad8:
            for nn in self.Elements.quad8[en]:
                fce()
        for en in self.Elements.tetra4:
            for nn in self.Elements.tetra4[en]:
                fce()
        for en in self.Elements.tetra10:
            for nn in self.Elements.tetra10[en]:
                fce()
        for en in self.Elements.hexa8:
            for nn in self.Elements.hexa8[en]:
                fce()
        for en in self.Elements.hexa20:
            for nn in self.Elements.hexa20[en]:
                fce()
        for en in self.Elements.penta6:
            for nn in self.Elements.penta6[en]:
                fce()
        for en in self.Elements.penta15:
            for nn in self.Elements.penta15[en]:
                fce()
        ...
        for nn in node_neighbours:
            ...
            for en in node_neighbours[nn]:
                dx = cg[en][0] - self.nodes[nn][0]
                dy = cg[en][1] - self.nodes[nn][1]
                dz = cg[en][2] - self.nodes[nn][2]
        ...
        for en in opt_domains:
            ...
            for nn in self.nodes:
                condition_x = down_x < self.nodes[nn][0] < up_x
                condition_y = down_y < self.nodes[nn][1] < up_y
                condition_z = down_z < self.nodes[nn][2] < up_z
...
# uses sectoring to prevent computing distance of far points
    def prepare1s(self, cg, r_min, opt_domains):
        # searching for elements neighbouring to every node
        node_neighbours = {}
        for meshType in self.Elements:
        ...
        sector_nodes = {}
        sector_elm = {}
        nodes_min = self.nodes[list(self.nodes.keys())[0]]  # initial values
        nodes_max = self.nodes[list(self.nodes.keys())[0]]
        for nn in self.nodes:
            nodes_min = [min(self.nodes[nn][0], nodes_min[0]), min(self.nodes[nn][1], nodes_min[1]), min(self.nodes[nn][2], nodes_min[2])]
            nodes_max = [max(self.nodes[nn][0], nodes_max[0]), max(self.nodes[nn][1], nodes_max[1]), max(self.nodes[nn][2], nodes_max[2])]
...
# function to filter sensitivity number to suppress checkerboard
    def run1(file_name, sensitivity_number, weight_factor_node, M, weight_factor_distance, near_nodes, self, opt_domains):
...
# uses sectoring to prevent computing distance of far points
    def prepare2s(self,cg, cg_min, cg_max, r_min, opt_domains, weight_factor2, near_elm):
        sector_elm = {}
        # preparing empty sectors
...
# uses sectoring to prevent computing distance of far points
    def prepare_morphology(self,cg, cg_min, cg_max, r_min, opt_domains, near_elm):
        sector_elm = {}
        # preparing empty sectors
        x = cg_min[0] + 0.5 * r_min
        while x <= cg_max[0] + 0.5 * r_min:
...
# uses sectoring to prevent computing distance of far points
    def prepare2s_casting(self,cg, r_min, opt_domains, above_elm, below_elm, casting_vector):
